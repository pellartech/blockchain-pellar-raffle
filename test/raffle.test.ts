...
describe('Raffle', () => {
  ...
  context('Test case for createRaffle', () => {
    it('createRaffle should be success', async () => {
      ...
      const raffle = await interactContract.getRaffleInfo(0)
      expect(raffle._totalWinners).equal(1000)
      expect(raffle._seeded).equal(false)
      expect(raffle._seedNumber).equal(0)
      expect(raffle._entries.length).equal(0)
      expect(raffle._winners.length).equal(0)
    })
  ...
  context('Test case for drawWinners', () => {
    it('drawWinners should be correct', async () => {
      ...
      const raffle = await interactContract.getRaffleInfo(0)
      expect(raffle._totalWinners).equal(100)
      expect(raffle._entries.length).equal(100)
      expect(raffle._winners.length).equal(100)
    })
  })
})
